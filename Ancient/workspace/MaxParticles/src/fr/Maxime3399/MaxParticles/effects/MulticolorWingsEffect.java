...
import fr.Maxime3399.MaxParticles.utils.RandomUtils;
import fr.Maxime3399.MaxParticles.utils.UtilParticles;

public class MulticolorWingsEffect {
		...
		for (boolean[] aShape : shape) {
			for (boolean anAShape : aShape) {
				if (anAShape) {
					...
					location.add(v2);
					for (int k = 0; k < 3; k++)
						UtilParticles.display(RandomUtils.randomColor(), RandomUtils.randomColor(), RandomUtils.randomColor(), location);
